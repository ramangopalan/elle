...
** Hacked by R <ramangopalan@gmail.com> for the Elle machine.
*/
...
#if defined (BUILD_LUA_COMPILER)

#include <elle/src/lua/lua.h>
#include <elle/src/lua/lauxlib.h>

#include <elle/src/lua/ldo.h>
#include <elle/src/lua/lfunc.h>
#include <elle/src/lua/lmem.h>
#include <elle/src/lua/lobject.h>
#include <elle/src/lua/lopcodes.h>
#include <elle/src/lua/lstring.h>
#include <elle/src/lua/lundump.h>

#if defined (BUILD_LUA_COMPILER)
#include <setjmp.h>
#define FATAL_RET_VALUE  -1		/* used in fatal() */
#define CANNOT_RET_VALUE -2		/* used in cannot() */
#define USAGE_RET_VALUE  -3		/* used in usage() */
static jmp_buf ebuf;			/* for 'env' in setjmp & longjmp */
#endif // #if BUILD_LUA_COMPILER

#if defined (BUILD_LUA_COMPILER)
#define PROGNAME	"/mmc/luac"     /* default program name for luac running on MCU */
#else
#define PROGNAME	"luac"		/* default program name */
#endif /* #if defined (BUILD_LUA_COMPILER) */
...
#endif
{
 ...
 /*
 * If we're using this on the MCU,
 * set these variables to their
 * default values before return;
 */
